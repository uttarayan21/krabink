...
// symbology. The first recognized `pendant://pair` URI is handed to the
// caller and the sheet closes; non-pendant codes are ignored in place.
    ...
    var body: some View {
        NavigationStack {
            ...
            .safeAreaInset(edge: .bottom) {
                if rejected {
                    Text("that code is not a pendant pairing QR")
                        .font(.footnote)
                        .foregroundStyle(.red)
                        .padding(.bottom, 8)
                }
