...
// through the same adoptPair() path as a scanned krabink:// URL.
//
// Needs `krabink-server --dev` running and its URI in KRABINK_TEST_PAIR.

import XCTest

final class PairUITests: XCTestCase {
    func testPairURIConnectsAndShowsQR() {
        // The URI `krabink-server --dev` (or a desktop) printed.
        let env = ProcessInfo.processInfo.environment
        let uri = env["KRABINK_TEST_PAIR"] ?? ""
        XCTAssertFalse(uri.isEmpty, "set KRABINK_TEST_PAIR to a krabink://pair URI")
