...
impl From<krabink_local::Route> for Route {
    fn from(r: krabink_local::Route) -> Self {
        match r {
            krabink_local::Route::Direct(addr) => Self::Direct {
                addr: addr.to_string(),
            },
            krabink_local::Route::Relay(url) => Self::Relay {
                url: url.to_string(),
            },
        }
    }
...
impl PeerKind {
    ...
    pub(crate) fn from_local(k: krabink_local::PeerKind) -> Option<Self> {
        Some(match k {
            krabink_local::PeerKind::Local => return None,
            krabink_local::PeerKind::Replica => Self::Replica,
            krabink_local::PeerKind::Desktop => Self::Desktop,
            krabink_local::PeerKind::Tablet => Self::Tablet,
            krabink_local::PeerKind::Unknown => Self::Unknown,
        })
    }
}
...
impl PeerInfo {
    pub(crate) fn from_status(s: krabink_local::PeerStatus) -> Option<Self> {
        let kind = PeerKind::from_local(s.kind)?;
        let (connected, route, error) = match s.state {
            krabink_local::PeerState::Connecting => (false, None, None),
            krabink_local::PeerState::Connected { route } => (true, route.map(Into::into), None),
            krabink_local::PeerState::Fatal { message } => (false, None, Some(message)),
        };
        Some(Self {
