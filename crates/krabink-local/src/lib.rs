//! Krabink sync node: the one networking stack every device runs.
//!
...
//! the sans-io [`krabink_core::ServerSession`]; outbound ones are driven
//! with [`krabink_core::ClientSession`]; the app on this device talks to its
...
/// Application-level protocol id every krabink node accepts.
pub const ALPN: &[u8] = b"krabink/sync/1";
