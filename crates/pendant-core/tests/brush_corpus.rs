...
//! measured next to the EMA one, and `PENDANT_METRICS_JSON=path` writes
...
use pendant_core::{InputModelKind, corpus};
...
fn recorded_strokes_replay_within_bounds() {
    ...
    if let Ok(path) = std::env::var("PENDANT_METRICS_JSON") {
