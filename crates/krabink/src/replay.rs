...
async fn replay(args: ReplayArgs) -> Result<()> {
    let info = PairInfo::parse(&args.pair)
        .ok_or_else(|| crate::errors::Report::new(Error).attach("not a krabink://pair URI"))?;
    ...
    let dir = std::env::temp_dir().join(format!("krabink-replay-{}", ulid::Ulid::new()));
    ...
    let mut session = krabink_core::ClientSession::new(device, info.token.clone());
...
async fn pump_until(
    ...
    loop {
        ...
        let server_msg = krabink_core::ServerMsg::decode(&frame)
            .change_context(Error)
