...
impl Docs {
    pub fn load(store: Store) -> krabink_core::Result<Self> {
    ...
    pub fn open_note(&mut self, id: NoteId) -> krabink_core::Result<&NoteDoc> {
    ...
    pub fn create_note(&mut self) -> krabink_core::Result<(NoteId, Vec<u8>, Vec<u8>)> {
        let id = NoteId::new();
    ...
    pub fn register_device(
        ...
        let before = self.workspace.version();
        self.workspace.upsert_device(&krabink_core::DeviceMeta {
            id,
            name: name.into(),
    ...
    pub fn refresh_meta(&mut self, id: NoteId) -> krabink_core::Result<MetaRefresh> {
    ...
    fn persist(&self, key: DocKey, payload: &[u8]) -> krabink_core::Result<()> {
