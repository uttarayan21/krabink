...
//! `KRABINK_UPDATE_GOLDEN=1 cargo test -p krabink-core --test golden_mesh`
...
fn mesh(ink: &Ink<'static>, samples: &[RawSample]) -> InkMesh {
    ...
    ink.mesh(
        &modeler.finish(),
        krabink_core::StrokeEnd::Complete,
        DEFAULT_TOLERANCE,
    )
}
...
fn golden_meshes_match() {
    ...
    if std::env::var_os("KRABINK_UPDATE_GOLDEN").is_some() {
        std::fs::write(GOLDEN, &text).expect("write golden file");
        return;
    }
    let expected = std::fs::read_to_string(Path::new(GOLDEN))
        .expect("tests/golden/mesh.txt exists; run with KRABINK_UPDATE_GOLDEN=1 to create it");
    ...
    assert!(
        ...
        "golden meshes changed; if intended, regenerate with KRABINK_UPDATE_GOLDEN=1\n{}",
        mismatched.join("\n")
    );
}
