...
//! ```
//!
//! Version 1 files have no `# krabink-stroke` line and four columns per
...
pub fn parse(text: &str) -> Result<Recording> {
    ...
    for (n, line) in text.lines().enumerate() {
        ...
        if let Some(rest) = line.strip_prefix("# krabink-stroke v") {
...
mod tests {
    ...
    fn rejects_bad_rows_and_tools() {
        ...
        assert!(parse("# krabink-stroke vX\n").is_err());
    }
}
